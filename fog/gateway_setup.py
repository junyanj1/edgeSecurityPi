...
def on_message(unused_client, unused_userdata, message):
    ...
    try:
        ...
        # elif payload == 'SHIT' or payload == b'SHIT':
        #     udpSerSock.sendto('SHIT'.encode('utf8'), client_addr)
        # else:
...
def main():
    ...
    while True:
        ...
        if action == 'event':
            ...
            _, mid = client.subscribe(subscribe_topic, qos=0)
            message = template.format(device_id, 'subscribe')
            gateway_state.subscriptions[subscribe_topic] = client_addr

#########################For Commands#################################
            print('subscribe commands for {}'.format(device_id))
            subscribe_topic = '/devices/{}/commands/#'.format(device_id)
            client.subscribe(subscribe_topic,qos=0)
            gateway_state.subscriptions[subscribe_topic[:-2]] = client_addr
            #############################################################
